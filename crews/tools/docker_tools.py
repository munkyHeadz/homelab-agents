...
def list_docker_images(show_all: bool = False) -> str:
    ...
    try:
        ...
        for img in images:
            ...
            created = datetime.fromtimestamp(img.attrs['Created']).strftime('%Y-%m-%d %H:%M')

            # Check if dangling (no tags)
            tags = img.tags
            if not tags or tags == ['<none>:<none>']:
                dangling_images.append(f"  {img.short_id}: {size_human} (created {created})")
            else:
    ...
    try:
        ...
        if network_name:
            ...
            for net in networks:
                ...
                output.append(f"• {name} ({driver}, {scope}): {container_count} containers")

            output.append("")
            output.append("💡 Tip: Use inspect_docker_network('network_name') for details")
...
def check_docker_volumes(show_usage: bool = True) -> str:
    ...
    try:
        ...
        if orphaned_volumes:
            ...
            output.append("")
            output.append("💡 Consider removing orphaned volumes with 'docker volume prune'")
...
def _format_container_stats(container_name: str, stats: dict) -> list:
    ...
    try:
        ...
        if mem_limit > 0:
            ...
            output.append(f"  Memory: {humanize.naturalsize(mem_usage)} / {humanize.naturalsize(mem_limit)} ({mem_percent:.1f}%)")
        else:
        ...
        if networks:
            ...
            output.append(f"  Network: RX {humanize.naturalsize(total_rx)}, TX {humanize.naturalsize(total_tx)}")
        ...
        if io_service_bytes:
            ...
            output.append(f"  Block I/O: Read {humanize.naturalsize(read_bytes)}, Write {humanize.naturalsize(write_bytes)}")
...
def check_docker_system_health() -> str:
    ...
    try:
        ...
        if images_size:
            ...
            active_img_size = sum(img.get('Size', 0) for img in images_size if not img.get('Containers', 0) == -1)
            reclaimable = total_img_size - active_img_size
            output.append(f"  Images: {humanize.naturalsize(total_img_size)} "
        ...
        if volumes_usage:
            total_vol_size = sum(v.get('UsageData', {}).get('Size', 0) for v in volumes_usage)
        ...
        if build_cache:
            ...
            reclaimable_cache = sum(b.get('Size', 0) for b in build_cache if b.get('Shared', False) == False)
            output.append(f"  Build Cache: {humanize.naturalsize(total_cache_size)} "
        ...
        if images_size:
            if reclaimable > 1024 * 1024 * 1024:  # > 1GB
                warnings.append(f"⚠️ {humanize.naturalsize(reclaimable)} of image space can be reclaimed")

        # Check for stopped containers
        if containers_stopped > 5:
            warnings.append(f"⚠️ {containers_stopped} stopped containers (consider cleanup)")
        ...
        if warnings:
            ...
            output.append("")
            output.append("💡 Use prune_docker_images and docker system prune for cleanup")
        else:
