...
def _make_unifi_local_request(endpoint: str, method: str = "GET", data: dict = None) -> dict:
...
def check_ap_health(ap_name: Optional[str] = None) -> str:
    ...
    try:
        ...
        if not aps:
            return f"✗ No Access Points found{f' matching: {ap_name}' if ap_name else ''}"
        ...
        for ap in aps:
            ...
            if state == 1:
                ...
                for radio in radio_table:
                    ...
                    result += f"    {radio_name}: Channel {channel}, Power {tx_power}dBm, "
        ...
        if e.response.status_code == 401:
            return "✗ UniFi API authentication failed - API key may be invalid or expired"
...
def monitor_network_clients(show_details: bool = False) -> str:
    ...
    try:
        ...
        if wireless:
            ...
            for ap_mac, ap_client_list in sorted(ap_clients.items(), key=lambda x: len(x[1]), reverse=True):
                # Try to get AP name
    ...
    try:
        ...
        for gw in gateways:
            ...
            if state == 1:
                ...
                result += "WAN1:\n"
                result += f"  Status: {'🟢 Connected' if wan1_up else '🔴 Disconnected'}\n"
                ...
                if speedtest:
                    download = speedtest.get('xput_download', 0) / 1000  # Convert to Mbps
...
def monitor_switch_ports(switch_name: Optional[str] = None) -> str:
    ...
    try:
        ...
        if switch_name:
            switches = [s for s in switches if s.get('name', '').lower() == switch_name.lower()]
        ...
        for switch in switches:
            ...
            result += f"🔌 {name} ({model}): {'🟢 Online' if state == 1 else '🔴 Offline'}\n"
            ...
            if state == 1:
                ...
                poe_active = sum(1 for p in port_table if p.get('poe_mode') == 'auto' and p.get('up', False))
                ...
                # Show port errors
                error_ports = [p for p in port_table if p.get('rx_errors', 0) > 100 or p.get('tx_errors', 0) > 100]
                if error_ports:
                    result += f"\n  ⚠️ Ports with Errors:\n"
                ...
                if busy_ports:
                    ...
                    for port in busy_ports:
                        ...
                        result += f"    {name_port}: RX {rx_mb:.1f}MB, TX {tx_mb:.1f}MB\n"
